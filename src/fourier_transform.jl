...
"""
function fourier_transform(kdata::AbstractArray{T}, c::Circle, gradient_delay_us) where T
    n_grid = c[:n_frequency]
    dft_matrix = get_dft_matrix(c, n_grid, gradient_delay_us, T)
...
"""
function get_dft_matrix(circle::Circle, n_grid, gradient_delay_us, datatype)
    xy = datatype(read_first_kspace_coordinate_normalized(circle))
    gradient_delay_rad = gradient_delay_to_rad(gradient_delay_us, circle)
    return calculate_dft_matrix(xy, circle[:n_points_on_circle], n_grid, gradient_delay_rad)
end

@memoize function calculate_dft_matrix(xy::Complex, n_points_on_circle, n_grid, gradient_delay_rad)
    kspace_coordinates = construct_circle_coordinates(xy, n_points_on_circle, gradient_delay_rad)
